...
class PointROIMinMax(PointROI):
    ...
    def __init__(self, caseArray, index, viewRange, pos, size=None, radius=None, **args):
        ...
        PointROI.__init__(self, caseArray, self.index, viewRange, pos, size, **args)
        self.maxBounds = {}        
        ...
        if self.index == 0 and self.pairIndex == 0:
            ...
            if self.pairIndex == 0:
                self.maxBounds["left"] = viewRange[0][0] if viewRange[0][0] > self.caseArray[self.index - 1][-1]*250 else self.caseArray[self.index - 1][-1]*250
            else:
        ...
        if self.index == len(self.caseArray) - 1 and self.pairIndex == 1:
            ...
            if self.pairIndex == 0:
                ...
                self.maxBounds["right"] = viewRange[0][1] if viewRange[0][1] < self.caseArray[self.index + 1][0]*250 else self.caseArray[self.index + 1][0]*250
